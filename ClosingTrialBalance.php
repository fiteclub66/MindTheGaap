...
?>
<?php include ($_SERVER['DOCUMENT_ROOT']."/includes/dcf_Populate_ClosingTrialBalance.php"); ?>

<html lang="en">
	<head>
		<title>Closing Trial Balance</title>
...
-->
			...
			<form action="ClosingTrialBalance.php" method="post">
			<div class="row">
						...
						<tbody style="margin-bottom: 10px;">
							<?php 
								...
								if ($result->num_rows > 0) {
									...
									$ridiculousSQL = "SELECT DISTINCT Journals.accountName, IFNULL((SELECT SUM(Journals.amount) FROM Journals WHERE Journals.creditDebit = 'Debit' AND Journals.accountSystemId = '$currentSystemId' AND Journals.status = 'Approved' AND Journals.date <= '$selectedDate' AND (Journals.type='REG' OR Journals.type='ADJ' OR Journals.type='CLO') GROUP BY Journals.accountName), 0) - IFNULL((SELECT SUM(Journals.amount) FROM Journals WHERE Journals.creditDebit = 'Credit' AND Journals.accountSystemId = '$currentSystemId' AND Journals.status = 'Approved' AND Journals.date <= '$selectedDate' AND (Journals.type='REG' OR Journals.type='ADJ' OR Journals.type='CLO') GROUP BY Journals.accountName), 0) AS 'balance', Accounts.normalSide FROM Journals, Accounts WHERE Journals.accountSystemId = '$currentSystemId' AND Journals.accountSystemId = Accounts.systemId";
